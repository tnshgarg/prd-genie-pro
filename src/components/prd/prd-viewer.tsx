...
export function PRDViewer({ prd }: PRDViewerProps) {
  ...
  const handleExport = async (format: 'markdown' | 'txt') => {
    try {
      const response = await fetch(`/api/prds/${prd.id}/export?format=${format}`)
      
      if (!response.ok) {
        throw new Error('Failed to export PRD')
      }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex items-start justify-between">
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                {prd.category && <Badge variant="secondary">{prd.category}</Badge>}
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-gray-700 whitespace-pre-wrap">{prd.original_idea}</p>
        </CardContent>
      </Card>
